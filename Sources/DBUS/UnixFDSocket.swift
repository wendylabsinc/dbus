  ...
  struct UnixFDSocket: Sendable {
    ...
    private func sendChunk(bytes: UnsafeRawBufferPointer, fds: [CInt]) throws -> Int {
      var iov = iovec(
        iov_base: UnsafeMutableRawPointer(mutating: bytes.baseAddress), iov_len: bytes.count)
      var msg = msghdr()
  ...
  private func cmsgFirstHeader(_ msg: UnsafeMutablePointer<msghdr>) -> UnsafeMutablePointer<
    cmsghdr
  >? {
