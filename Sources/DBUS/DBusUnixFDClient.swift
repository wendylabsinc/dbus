...
public enum DBusUnixFDClient {
  ...
  public static func withConnection<R: Sendable>(
    ...
    DispatchQueue.global(qos: .utility).async { [initialBytes] in
      ...
      var fdQueue: [CInt] = []
      do {
        while let message = try readSocket.receiveMessage(buffer: &decodeBuffer, fdQueue: &fdQueue) {
  ...
  private static func authenticate(
    ...
    while true {
      ...
      if line.starts(with: "REJECTED") || line.starts(with: "ERROR") {
        let trimmed = line.starts(with: "REJECTED")
...
public enum DBusUnixFDClient {
  ...
  public actor Send {
    ...
    public func send(_ request: DBusRequest) async throws -> UInt32 { throw DBusError.unixFdUnsupported }
  ...
  public actor Connection: Sendable {
    ...
    public func send(_ request: DBusRequest, timeoutNanoseconds: UInt64?) async throws -> DBusMessage? {
      _ = timeoutNanoseconds
      throw DBusError.unixFdUnsupported
    }
    public func setMessageHandler(_ handler: @escaping @Sendable (DBusMessage) async -> Void) async {
      _ = handler
    }
  }
